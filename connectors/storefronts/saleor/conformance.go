...
func (candidate *ConformanceCandidate) Probe(_ context.Context, request conformance.ProbeRequest) (conformance.ProbeResult, error) {
	...
	case conformance.ProbeRateLimited:
		remote, _ := sdk.NewRemoteError(sdk.ErrorRateLimited, "rate_limited", "req-saleor", time.Second)
		return conformance.ProbeResult{}, remote
...
func (candidateTransport) Do(_ context.Context, request Request) (Response, error) {
	if len(request.Bearer) == 0 {
		return Response{StatusCode: 200, Body: []byte(`{"errors":[{"message":"Permission denied","extensions":{"exception":{"code":"PermissionDenied"}}}]}`)}, nil
	}
	if request.Method == "POST" && request.Host == "shop.example.com" && request.Path == "/graphql/" {
		return Response{StatusCode: 200, Body: []byte(`{"data":{"productVariants":{"edges":[],"pageInfo":{"hasNextPage":false,"endCursor":""}}}}`)}, nil
	}
...
func (candidateSecrets) UseSecret(_ context.Context, _ sdk.SecretReference, callback func([]byte) error) error {
	...
	value := []byte("candidateapptoken0123456789abcdef0123456789")
	defer clear(value)

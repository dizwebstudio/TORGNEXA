...
var (
	ErrTransportMissing   = errors.New("magento: transport missing")
	ErrInvalidResponse    = errors.New("magento: invalid response")
...
func Manifest() sdk.Manifest {
	return sdk.Manifest{
		ID: "magento", Name: "Magento (Adobe Commerce)", Family: sdk.FamilyStorefront, Version: "1.0.0", SDKVersion: sdk.SDKMajor,
...
// credentials holds only the Integration access token (Magento Admin >
// System > Integrations): once an integration is created and activated,
// Magento issues one long-lived access token used directly as a bearer
// credential on ordinary REST calls — no runtime OAuth1.0a signing or
// refresh is needed on this connector's side, unlike Shopware's
...
func (connector *Connector) Health(ctx context.Context, account sdk.Account, runtime sdk.Runtime) (sdk.Health, error) {
	...
	err = connector.withCredentials(ctx, runtime, account.SecretReference, func(credential credentials) error {
		query := searchCriteriaQuery(1, 1, nil)
		_, callErr := connector.call(ctx, configuration, credential, "GET", "/products", query, nil)
		return callErr
	})
	if err == nil {

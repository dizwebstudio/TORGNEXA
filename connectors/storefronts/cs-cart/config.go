...
var (
	ErrConfigurationMissing = errors.New("cs-cart: configuration missing")
	ErrInvalidConfiguration = errors.New("cs-cart: invalid configuration")
)
...
// Configuration contains only non-secret settings for a CS-Cart store.
...
// apiPath uses the API 2.0 route recommended by CS-Cart documentation.
func (c Configuration) apiPath(suffix string) string { return c.BasePath + "/api/2.0" + suffix }

...
// ConformanceCandidate supplies deterministic SDK and sandbox behavior for
// admission tests without contacting a local Ollama daemon.
...
func NewConformanceCandidate(exe string) (*ConformanceCandidate, error) {
	manifest := Manifest()
	manifest.Auth = []sdk.AuthRequirement{{Kind: sdk.AuthAPIKey, SecretClass: "ai_provider_credential", Required: true}}
	f, err := conformance.NewSandboxFixture(manifest, exe)
	if err != nil {
		return nil, err
	}
	c := New(candidateTransport{}, func() time.Time { return time.Date(2026, 8, 28, 2, 0, 0, 0, time.UTC) })
...
func (c *ConformanceCandidate) Account(t conformance.Tenant) sdk.Account {
	...
	return sdk.Account{ID: "ollama-conformance", OrganizationID: t.OrganizationID, WorkspaceID: t.WorkspaceID, ConnectorID: "ollama", Family: sdk.FamilyAI, Status: sdk.AccountActive, SecretReference: "sec:v1:0123456789abcdef0123456789abcdef", Version: 1, Health: sdk.Health{Status: sdk.HealthUnknown}, CreatedAt: at, UpdatedAt: at}
}
...
func candidateRemote(cat sdk.ErrorCategory, code string, retry time.Duration) error {
	e, _ := sdk.NewRemoteError(cat, code, "ollama-conformance", retry)
	return e
}
...
func (candidateSecrets) UseSecret(_ context.Context, _ sdk.SecretReference, cb func([]byte) error) error {
	return cb([]byte("conformance-credential"))
}

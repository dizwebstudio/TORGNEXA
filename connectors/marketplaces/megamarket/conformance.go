...
func NewConformanceCandidate(emulatorExecutable string) (*ConformanceCandidate, error) {
	...
	config := Configuration{MerchantID: 10001, Scheme: SchemeDBS, Warehouses: []Warehouse{{ID: "WH-01", Name: "Main"}}}
...
func (candidate *ConformanceCandidate) Probe(_ context.Context, request conformance.ProbeRequest) (conformance.ProbeResult, error) {
	...
	case conformance.ProbeRateLimited:
		remote, _ := sdk.NewRemoteError(sdk.ErrorRateLimited, "rate_limited", "mm-conformance", 1500*time.Millisecond)
		return conformance.ProbeResult{}, remote
...
func (candidateTransport) Do(_ context.Context, request Request) (Response, error) {
	if len(request.MerchantToken) == 0 {
		return Response{StatusCode: 401}, nil
	}
	if request.Method == "POST" && request.Host == apiHost && request.Path == "/api/merchantIntegration/assortment/v1/card/getAttributes" {
		return Response{StatusCode: 200, Body: []byte(`{"success":true,"data":{"items":[],"searchAfter":""}}`)}, nil
	}
...
func (candidateSecrets) UseSecret(_ context.Context, _ sdk.SecretReference, callback func([]byte) error) error {
	...
	value := []byte("synthetic-megamarket-token-0123456789abcdef")
	defer clear(value)

...
func (connector *Connector) ReadAdvertisingCampaigns(ctx context.Context, account sdk.Account, runtime sdk.Runtime, page sdk.PageRequest) (sdk.AdvertisingCampaignPage, error) {
	if connector == nil || connector.transport == nil || runtime == nil || runtime.Secrets() == nil || sdk.ValidateAccountAgainstManifest(account, Manifest()) != nil || sdk.RequireCapability(Manifest(), "ads.read") != nil || page.Validate(100) != nil || page.Cursor != "" {
	...
	err := runtime.Secrets().UseSecret(ctx, account.SecretReference, func(secret []byte) error {
		...
		var parsed promotionCountResponse
		if len(response.Body) == 0 || json.Unmarshal(response.Body, &parsed) != nil {
			return ErrInvalidResponse
		}
		for _, group := range parsed.Adverts {
			for _, item := range group.AdvertList {
				if item.AdvertID <= 0 {
					return ErrInvalidResponse
				}
				observed := connector.now().UTC()
				if item.ChangeTime != "" {
					if parsedTime, parseErr := time.Parse(time.RFC3339Nano, item.ChangeTime); parseErr == nil {
						observed = parsedTime.UTC()
					}
				}
				output.Items = append(output.Items, sdk.RemoteCampaign{RemoteID: strconv.FormatInt(item.AdvertID, 10), Name: "WB campaign " + strconv.FormatInt(item.AdvertID, 10), Status: wbCampaignStatus(group.Status), Currency: "RUB", ObservedAt: observed})
			}
		}
...
func wbCampaignStatus(status int) string {
	switch status {
	case 9:
		return "active"
	case 11:
...
func (connector *Connector) readAdvertisingStats(ctx context.Context, account sdk.Account, runtime sdk.Runtime, query sdk.AdvertisingQuery) ([]sdk.RemoteAdSpendFact, []sdk.RemoteAdPerformanceFact, error) {
	if connector == nil || connector.transport == nil || runtime == nil || runtime.Secrets() == nil || sdk.ValidateAccountAgainstManifest(account, Manifest()) != nil || sdk.RequireCapability(Manifest(), "ads.read") != nil || query.Validate(50, 100) != nil {
	...
	err := runtime.Secrets().UseSecret(ctx, account.SecretReference, func(secret []byte) error {
		response, callErr := connector.transport.Do(ctx, Request{Method: "GET", Host: advertisingHost, Path: "/adv/v3/fullstats", Query: parameters, Token: secret})
		if callErr != nil {
			return normalizedTransportError()
		}
		if remote := normalizeHTTP(response); remote != nil {
		...
		var parsed []wbAdvertisingStat
		if len(response.Body) == 0 || json.Unmarshal(response.Body, &parsed) != nil {
			return ErrInvalidResponse
		}
		for _, campaign := range parsed {
			for _, day := range campaign.Days {
				start, err := parseWBDate(day.Date)
				if err != nil {
					return ErrInvalidResponse
				}
				end := start.Add(24 * time.Hour)
				views, clicks, orders, sum, sumPrice := decimalInt(day.Views), decimalInt(day.Clicks), decimalInt(day.Orders), minorFromDecimal(day.Sum.String()), minorFromDecimal(day.SumPrice.String())
				hasProducts := false
				for _, app := range day.Apps {
					for _, product := range app.NM {
						hasProducts = true
						sku := strconv.FormatInt(product.NmID, 10)
						productViews, productClicks, productOrders := decimalInt(product.Views), decimalInt(product.Clicks), decimalInt(product.Orders)
						productSpend, productRevenue := minorFromDecimal(product.Sum.String()), minorFromDecimal(product.SumPrice.String())
						remoteID := strconv.FormatInt(campaign.AdvertID, 10) + ":" + start.Format("2006-01-02") + ":" + sku
						spends = append(spends, sdk.RemoteAdSpendFact{RemoteFactID: remoteID, CampaignID: strconv.FormatInt(campaign.AdvertID, 10), SKU: sku, PeriodStart: start, PeriodEnd: end, AmountMinor: productSpend, Currency: "RUB", ObservedAt: connector.now().UTC(), EffectiveAt: start, Quality: "confirmed"})
						performance = append(performance, sdk.RemoteAdPerformanceFact{RemoteFactID: remoteID, CampaignID: strconv.FormatInt(campaign.AdvertID, 10), SKU: sku, Impressions: productViews, Clicks: productClicks, Orders: productOrders, RevenueMinor: productRevenue, Currency: "RUB", PeriodStart: start, PeriodEnd: end, ObservedAt: connector.now().UTC(), EffectiveAt: start, Quality: "confirmed"})
					}
				}
				if !hasProducts {
					remoteID := strconv.FormatInt(campaign.AdvertID, 10) + ":" + start.Format("2006-01-02")
					spends = append(spends, sdk.RemoteAdSpendFact{RemoteFactID: remoteID, CampaignID: strconv.FormatInt(campaign.AdvertID, 10), PeriodStart: start, PeriodEnd: end, AmountMinor: sum, Currency: "RUB", ObservedAt: connector.now().UTC(), EffectiveAt: start, Quality: "confirmed"})
...
func (connector *Connector) ReadAdvertisingSpend(ctx context.Context, account sdk.Account, runtime sdk.Runtime, query sdk.AdvertisingQuery) (sdk.AdvertisingSpendPage, error) {
	spends, _, err := connector.readAdvertisingStats(ctx, account, runtime, query)
	return sdk.AdvertisingSpendPage{Items: spends}, err
}
...
func decimalInt(value json.Number) int64 {
...
func minorFromDecimal(value string) int64 {
	...
	cents := new(big.Int)
	if frac != "" {
		if _, ok := cents.SetString(frac, 10); !ok {
			return 0
		}
	}

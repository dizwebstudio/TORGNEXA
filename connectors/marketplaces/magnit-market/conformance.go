...
func NewConformanceCandidate(emulatorExecutable string) (*ConformanceCandidate, error) {
	...
	configuration := Configuration{ShopID: 42, StockType: StockTypeFBS, OrderWindowDays: 30}
	return &ConformanceCandidate{
		SandboxFixture: fixture,
		connector: New(candidateTransport{}, staticConformanceConfig{configuration}, func() time.Time {
			return time.Date(2026, 8, 10, 19, 0, 0, 0, time.UTC)
		}),
		idempotency: map[string]string{},
		webhooks:    map[string]string{},
	}, nil
...
func (candidate *ConformanceCandidate) Account(tenant conformance.Tenant) sdk.Account {
	...
	return sdk.Account{
		ID: "magnit-market-conformance", OrganizationID: tenant.OrganizationID, WorkspaceID: tenant.WorkspaceID,
		ConnectorID: Manifest().ID, Family: sdk.FamilyMarketplace, Status: sdk.AccountActive,
		SecretReference: "sec:v1:0123456789abcdef0123456789abcdef", Version: 1,
		Health: sdk.Health{Status: sdk.HealthUnknown}, CreatedAt: created, UpdatedAt: created,
	}
}
...
func (candidate *ConformanceCandidate) Probe(_ context.Context, request conformance.ProbeRequest) (conformance.ProbeResult, error) {
	...
	case conformance.ProbeRateLimited:
		remote, _ := sdk.NewRemoteError(sdk.ErrorRateLimited, "rate_limited", "magnit-conformance", 1500*time.Millisecond)
		return conformance.ProbeResult{}, remote
...
func (candidateTransport) Do(_ context.Context, request Request) (Response, error) {
	...
	if request.Method == "GET" && request.Host == apiHost && request.Path == "/api/seller/v1/shops" {
		return Response{StatusCode: 200, Body: []byte(`[{"id":42,"title":"Conformance shop"}]`)}, nil
	}
...
func (candidateSecrets) UseSecret(_ context.Context, _ sdk.SecretReference, callback func([]byte) error) error {
	...
	value := []byte("synthetic-magnit-market-key-0123456789abcdef")
	defer clear(value)

...
var (
	ErrConfigurationMissing = errors.New("shopware: configuration missing")
	ErrInvalidConfiguration = errors.New("shopware: invalid configuration")
)
...
// Configuration is admin-supplied non-secret runtime config: Shopware is
// self-hosted, so there is no fixed host, the same shape
// WooCommerce/OpenCart/PrestaShop/Medusa already use.
...
func (configuration Configuration) apiPath(suffix string) string {
	return configuration.BasePath + "/api" + suffix
}

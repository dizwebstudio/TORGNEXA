...
func (c *ConformanceCandidate) Account(t conformance.Tenant) sdk.Account {
	...
	return sdk.Account{ID: "bitrix24-conformance", OrganizationID: t.OrganizationID, WorkspaceID: t.WorkspaceID, ConnectorID: Manifest().ID, Family: sdk.FamilyCRM, Status: sdk.AccountActive, SecretReference: "sec:v1:0123456789abcdef0123456789abcdef", Version: 1, Health: sdk.Health{Status: sdk.HealthUnknown}, CreatedAt: at, UpdatedAt: at}
}
...
func (c *ConformanceCandidate) Probe(_ context.Context, r conformance.ProbeRequest) (conformance.ProbeResult, error) {
	...
	case conformance.ProbeRateLimited:
		remote, _ := sdk.NewRemoteError(sdk.ErrorRateLimited, "rate_limited", "req-b24", time.Second)
		return conformance.ProbeResult{}, remote
...
func (candidateTransport) Do(_ context.Context, r Request) (Response, error) {
	if len(r.Bearer) == 0 {
		return Response{StatusCode: 401, Body: []byte(`{"error":"NO_AUTH"}`)}, nil
	}
	if r.Path == "/rest/crm.item.fields" {
		return Response{StatusCode: 200, Body: []byte(`{"result":{"fields":{"id":{"type":"integer"}}}}`)}, nil
	}
	return Response{StatusCode: 404, Body: []byte(`{"error":"NOT_FOUND"}`)}, nil
}

...
func NewConformanceCandidate(exe string) (*ConformanceCandidate, error) {
	...
	return &ConformanceCandidate{SandboxFixture: f, connector: New(candidateTransport{}, candidateConfig{}, func() time.Time { return time.Date(2026, 8, 11, 19, 0, 0, 0, time.UTC) }), idem: map[string]string{}, hooks: map[string]string{}}, nil
}
...
func (c *ConformanceCandidate) Account(t conformance.Tenant) sdk.Account {
	at := time.Date(2026, 8, 11, 18, 0, 0, 0, time.UTC)
	return sdk.Account{ID: "avito-conformance", OrganizationID: t.OrganizationID, WorkspaceID: t.WorkspaceID, ConnectorID: "avito", Family: sdk.FamilyClassified, Status: sdk.AccountActive, SecretReference: "sec:v1:0123456789abcdef0123456789abcdef", Version: 1, Health: sdk.Health{Status: sdk.HealthUnknown}, CreatedAt: at, UpdatedAt: at}
}
...
func (c *ConformanceCandidate) Probe(_ context.Context, r conformance.ProbeRequest) (conformance.ProbeResult, error) {
	...
	case conformance.ProbeRateLimited:
		return conformance.ProbeResult{}, remote(sdk.ErrorRateLimited, "rate_limited", "avito", time.Second)
...
func replay(m map[string]string, k, s string) (conformance.ProbeResult, error) {
	if k == "" {
		return conformance.ProbeResult{}, conformance.ErrInvalidCandidate
	}
	if f, ok := m[k]; ok {
...
func (candidateTransport) Do(_ context.Context, r Request) (Response, error) {
	...
	if r.Path == "/core/v1/accounts/self" {
		return Response{StatusCode: 200, Body: []byte(`{"id":42}`)}, nil
	}
...
func (candidateSecrets) UseSecret(_ context.Context, _ sdk.SecretReference, cb func([]byte) error) error {
	...
	b := []byte("synthetic-avito-oauth-access-token-0123456789")
	defer clear(b)
	return cb(b)
}

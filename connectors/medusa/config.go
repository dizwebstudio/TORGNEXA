...
var (
	ErrConfigurationMissing = errors.New("medusa: configuration missing")
	ErrInvalidConfiguration = errors.New("medusa: invalid configuration")
)
...
// Configuration is admin-supplied non-secret runtime config: Medusa is
// self-hosted, so there is no fixed host the way Shopify's OAuth flow has
// one, exactly the same shape WooCommerce/OpenCart/PrestaShop already use.
...
func (configuration Configuration) apiPath(suffix string) string {
	return configuration.BasePath + "/admin" + suffix
}

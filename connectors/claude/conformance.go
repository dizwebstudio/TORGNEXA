...
func NewConformanceCandidate(exe string) (*ConformanceCandidate, error) {
	...
	sandboxManifest.Auth = []sdk.AuthRequirement{{Kind: sdk.AuthAPIKey, SecretClass: "ai_provider_credential", Required: true}}
	f, err := conformance.NewSandboxFixture(sandboxManifest, exe)
	if err != nil {
		return nil, err
	}
	c := New(candidateTransport{}, func() time.Time { return time.Date(2026, 8, 20, 2, 0, 0, 0, time.UTC) })
...
func (c *ConformanceCandidate) Account(t conformance.Tenant) sdk.Account {
	...
	return sdk.Account{ID: "claude-conformance", OrganizationID: t.OrganizationID, WorkspaceID: t.WorkspaceID, ConnectorID: "claude", Family: sdk.FamilyAI, Status: sdk.AccountActive, SecretReference: "sec:v1:0123456789abcdef0123456789abcdef", Version: 1, Health: sdk.Health{Status: sdk.HealthUnknown}, CreatedAt: at, UpdatedAt: at}
}
...
func candidateRemote(category sdk.ErrorCategory, code string, retry time.Duration) error {
	e, _ := sdk.NewRemoteError(category, code, "claude-conformance", retry)
	return e
}

type candidateTransport struct{}

func (candidateTransport) Do(_ context.Context, _ Request) (Response, error) {
	raw, _ := json.Marshal(messageResponse{Model: DefaultHealthModel, Content: []contentBlock{{Type: "text", Text: "ok"}}})
...
func (candidateSecrets) UseSecret(_ context.Context, _ sdk.SecretReference, callback func([]byte) error) error {
	return callback([]byte("conformance-credential"))
}

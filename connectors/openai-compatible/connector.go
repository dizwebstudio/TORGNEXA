// Package openaicompatible implements the Connector SDK boundary for the
// OpenAI Chat Completions wire format (OpenAI itself, or any compatible
// gateway reachable at the account's configured host). All socket I/O is
...
var ErrTransportMissing = errors.New("openai-compatible: transport missing")

const DefaultHost = "api.openai.com"

// Request is a host-mediated HTTPS call: Host/Path/Headers/Body only, no
// scheme or socket handling, so this package never imports net/*.
type Request struct {
	Host    string
	Path    string
...
// Response is the raw result of one Request.
...
func Manifest() sdk.Manifest {
	manifest, _ := sdk.CatalogManifest("openai-compatible")
	return manifest
}
...
func (c *Connector) Health(ctx context.Context, account sdk.Account, runtime sdk.Runtime) (sdk.Health, error) {
	...
	err := runtime.Secrets().UseSecret(ctx, account.SecretReference, func(credential []byte) error {
		_, _, callErr := c.complete(ctx, "", "gpt-4o-mini", credential, "", "ping")
		return callErr
	})
	if err != nil {
...
// Complete sends one bounded analytics prompt through the account's stored
// credential and returns the provider's completion text. host overrides
// DefaultHost when the account configured an alternate OpenAI-compatible
// gateway (a bare hostname, never a full URL: this package cannot parse one).
...
func (c *Connector) complete(ctx context.Context, host, model string, credential []byte, systemPrompt, userPrompt string) (string, string, error) {
	if len(credential) == 0 || strings.TrimSpace(userPrompt) == "" {
		return "", "", errors.New("openai-compatible: invalid request")
	}
	if host == "" {
		host = DefaultHost
	}
	...
	if response.StatusCode < 200 || response.StatusCode >= 300 {
		return "", "", errors.New("openai-compatible: remote status not ok")
	}
	...
	if len(out.Choices) == 0 || strings.TrimSpace(out.Choices[0].Message.Content) == "" {
		return "", "", errors.New("openai-compatible: empty completion")
	}

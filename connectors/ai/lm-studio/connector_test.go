...
func (testSecrets) UseSecret(_ context.Context, _ sdk.SecretReference, fn func([]byte) error) error {
	return fn([]byte("lm-studio"))
}
func testAccount() sdk.Account {
	at := time.Date(2026, 8, 28, 0, 0, 0, 0, time.UTC)
	return sdk.Account{ID: "lm-studio-test", OrganizationID: "018f0e8b-8a58-7f42-8c2d-5c2f9b1a0001", WorkspaceID: "018f0e8b-8a58-7f42-8c2d-5c2f9b1a0002", ConnectorID: "lm-studio", Family: sdk.FamilyAI, Status: sdk.AccountActive, SecretReference: "sec:v1:0123456789abcdef0123456789abcdef", Version: 1, Health: sdk.Health{Status: sdk.HealthUnknown}, CreatedAt: at, UpdatedAt: at}
}
...
func TestCompleteUsesConfiguredLocalEndpoint(t *testing.T) {
	...
	text, model, err := New(transport, nil).Complete(context.Background(), testAccount(), testRuntime{}, "http://lm-studio:1234/v1", "local-model", "be terse", "summarize sales")
	if err != nil || text != "ok" || model != "local-model" {
		t.Fatalf("text=%q model=%q err=%v", text, model, err)
	}
	if transport.request.BaseURL != "http://lm-studio:1234/v1" || transport.request.Path != "/chat/completions" {

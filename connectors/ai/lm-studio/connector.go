// Package lmstudio implements the LM Studio OpenAI-compatible chat boundary.
...
var ErrTransportMissing = errors.New("lm-studio: transport missing")
...
// Connector is a stateless LM Studio adapter.
type Connector struct {
	transport Transport
	now       func() time.Time
}

// New constructs an LM Studio connector using a host-owned transport.
...
// Manifest returns the canonical LM Studio connector manifest.
func Manifest() sdk.Manifest                { manifest, _ := sdk.CatalogManifest("lm-studio"); return manifest }
...
func (c *Connector) complete(ctx context.Context, baseURL, model string, credential []byte, systemPrompt, userPrompt string) (string, string, error) {
	if len(credential) == 0 || strings.TrimSpace(userPrompt) == "" {
		return "", "", errors.New("lm-studio: invalid request")
	}
	...
	if response.StatusCode < 200 || response.StatusCode >= 300 {
		return "", "", errors.New("lm-studio: remote status not ok")
	}
	...
	if len(out.Choices) == 0 || strings.TrimSpace(out.Choices[0].Message.Content) == "" {
		return "", "", errors.New("lm-studio: empty completion")
	}

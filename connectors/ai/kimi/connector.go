// Package kimi implements the Connector SDK boundary for Moonshot AI's Kimi
// models. Kimi's chat-completions wire format is OpenAI-compatible; the
...
var ErrTransportMissing = errors.New("kimi: transport missing")

const DefaultHost = "api.moonshot.ai"
...
func Manifest() sdk.Manifest                { manifest, _ := sdk.CatalogManifest("kimi"); return manifest }
...
func (c *Connector) Health(ctx context.Context, account sdk.Account, runtime sdk.Runtime) (sdk.Health, error) {
	...
	err := runtime.Secrets().UseSecret(ctx, account.SecretReference, func(credential []byte) error {
		_, _, callErr := c.complete(ctx, "", "moonshot-v1-8k", credential, "", "ping")
		return callErr
	})
	if err != nil {
...
func (c *Connector) complete(ctx context.Context, host, model string, credential []byte, systemPrompt, userPrompt string) (string, string, error) {
	if len(credential) == 0 || strings.TrimSpace(userPrompt) == "" {
		return "", "", errors.New("kimi: invalid request")
	}
	if host == "" {
		host = DefaultHost
	}
	...
	if response.StatusCode < 200 || response.StatusCode >= 300 {
		return "", "", errors.New("kimi: remote status not ok")
	}
	...
	if len(out.Choices) == 0 || strings.TrimSpace(out.Choices[0].Message.Content) == "" {
		return "", "", errors.New("kimi: empty completion")
	}

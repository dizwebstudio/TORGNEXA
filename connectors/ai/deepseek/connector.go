// Package deepseek implements the Connector SDK boundary for DeepSeek's
// models. DeepSeek's chat-completions wire format is OpenAI-compatible; the
// request/response shapes are intentionally re-declared here rather than
// imported from connectors/ai/openai-compatible, since connector packages may
// not import each other. All socket I/O is host-mediated through Transport.
package deepseek
...
var ErrTransportMissing = errors.New("deepseek: transport missing")

const DefaultHost = "api.deepseek.com"
...
func Manifest() sdk.Manifest                { manifest, _ := sdk.CatalogManifest("deepseek"); return manifest }
...
func (c *Connector) Health(ctx context.Context, account sdk.Account, runtime sdk.Runtime) (sdk.Health, error) {
	...
	err := runtime.Secrets().UseSecret(ctx, account.SecretReference, func(credential []byte) error {
		_, _, callErr := c.complete(ctx, "", "deepseek-chat", credential, "", "ping")
		return callErr
	})
	if err != nil {
...
func (c *Connector) complete(ctx context.Context, host, model string, credential []byte, systemPrompt, userPrompt string) (string, string, error) {
	if len(credential) == 0 || strings.TrimSpace(userPrompt) == "" {
		return "", "", errors.New("deepseek: invalid request")
	}
	if host == "" {
		host = DefaultHost
	}
	...
	if response.StatusCode < 200 || response.StatusCode >= 300 {
		return "", "", errors.New("deepseek: remote status not ok")
	}
	...
	if len(out.Choices) == 0 || strings.TrimSpace(out.Choices[0].Message.Content) == "" {
		return "", "", errors.New("deepseek: empty completion")
	}

...
func NewConformanceCandidate(exe string) (*ConformanceCandidate, error) {
	m := Manifest()
	m.Auth = []sdk.AuthRequirement{{Kind: sdk.AuthAPIKey, SecretClass: "ai_provider_credential", Required: true}}
	f, err := conformance.NewSandboxFixture(m, exe)
	if err != nil {
		return nil, err
	}
	return &ConformanceCandidate{SandboxFixture: f, connector: New(candidateTransport{}, nil), idem: map[string]string{}, hooks: map[string]string{}}, nil
}
...
func (c *ConformanceCandidate) Account(t conformance.Tenant) sdk.Account {
	...
	return sdk.Account{ID: "grok-conformance", OrganizationID: t.OrganizationID, WorkspaceID: t.WorkspaceID, ConnectorID: "grok", Family: sdk.FamilyAI, Status: sdk.AccountActive, SecretReference: "sec:v1:0123456789abcdef0123456789abcdef", Version: 1, Health: sdk.Health{Status: sdk.HealthUnknown}, CreatedAt: at, UpdatedAt: at}
}
...
func candidateRemote(cat sdk.ErrorCategory, code string, retry time.Duration) error {
	e, _ := sdk.NewRemoteError(cat, code, "grok-conformance", retry)
	return e
}
...
func (candidateSecrets) UseSecret(_ context.Context, _ sdk.SecretReference, fn func([]byte) error) error {
	return fn([]byte("conformance-credential"))
}

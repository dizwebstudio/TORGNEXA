// Package ollama implements the Ollama OpenAI-compatible chat boundary.
// Network access is host-mediated through Transport; the connector never
// owns DNS, sockets or local-network policy.
package ollama
...
var ErrTransportMissing = errors.New("ollama: transport missing")

const (
	DefaultBaseURL     = "http://ollama:11434/v1"
	DefaultHealthModel = "llama3.2"
)
...
// Connector is a stateless Ollama adapter.
type Connector struct {
	transport Transport
	now       func() time.Time
}

// New constructs an Ollama connector using a host-owned transport.
...
// Manifest returns the canonical Ollama connector manifest.
func Manifest() sdk.Manifest { manifest, _ := sdk.CatalogManifest("ollama"); return manifest }

// Manifest returns the canonical Ollama connector manifest.
func (c *Connector) Manifest() sdk.Manifest { return Manifest() }

// Health performs one bounded non-streaming local completion.
...
// Complete sends one bounded text completion to the configured local server.
// An empty baseURL uses the Compose service name and port above.
...
func (c *Connector) complete(ctx context.Context, baseURL, model string, credential []byte, systemPrompt, userPrompt string) (string, string, error) {
	if len(credential) == 0 || strings.TrimSpace(userPrompt) == "" {
		return "", "", errors.New("ollama: invalid request")
	}
	...
	response, err := c.transport.Do(ctx, Request{BaseURL: baseURL, Path: "/chat/completions", Headers: map[string]string{
		"Authorization": "Bearer " + string(credential), "Content-Type": "application/json",
	}, Body: raw})
	if err != nil {
		return "", "", err
	}
	if response.StatusCode < 200 || response.StatusCode >= 300 {
		return "", "", errors.New("ollama: remote status not ok")
	}

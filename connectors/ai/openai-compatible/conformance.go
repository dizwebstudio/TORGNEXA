...
func NewConformanceCandidate(exe string) (*ConformanceCandidate, error) {
	// Task-064 sandbox probes exercise the generic secret boundary using a
	// synthetic secret that exists only inside the harness.
...
func (c *ConformanceCandidate) Account(t conformance.Tenant) sdk.Account {
	...
	return sdk.Account{ID: "openai-compatible-conformance", OrganizationID: t.OrganizationID, WorkspaceID: t.WorkspaceID, ConnectorID: "openai-compatible", Family: sdk.FamilyAI, Status: sdk.AccountActive, SecretReference: "sec:v1:0123456789abcdef0123456789abcdef", Version: 1, Health: sdk.Health{Status: sdk.HealthUnknown}, CreatedAt: at, UpdatedAt: at}
}
...
func candidateRemote(cat sdk.ErrorCategory, code string, retry time.Duration) error {
	e, _ := sdk.NewRemoteError(cat, code, "openai-compatible-conformance", retry)
	return e
}
...
func (candidateTransport) Do(_ context.Context, _ Request) (Response, error) {
	raw, _ := json.Marshal(chatResponse{Model: "gpt-conformance", Choices: []chatChoice{{Message: chatMessage{Role: "assistant", Content: "ok"}}}})

...
func NewConformanceCandidate(emulatorExecutable string) (*ConformanceCandidate, error) {
	...
	return &ConformanceCandidate{SandboxFixture: fixture, connector: New(candidateTransport{}, candidateConfiguration{}, func() time.Time { return time.Date(2026, 8, 12, 8, 0, 0, 0, time.UTC) }), idempotency: map[string]string{}, webhooks: map[string]string{}}, nil
}
...
func (candidate *ConformanceCandidate) Account(tenant conformance.Tenant) sdk.Account {
	...
	return sdk.Account{ID: "woo-conformance", OrganizationID: tenant.OrganizationID, WorkspaceID: tenant.WorkspaceID, ConnectorID: Manifest().ID, Family: sdk.FamilyMarketplace, Status: sdk.AccountActive, SecretReference: "sec:v1:0123456789abcdef0123456789abcdef", Version: 1, Health: sdk.Health{Status: sdk.HealthUnknown}, CreatedAt: at, UpdatedAt: at}
}
...
func (candidate *ConformanceCandidate) Probe(_ context.Context, request conformance.ProbeRequest) (conformance.ProbeResult, error) {
	...
	case conformance.ProbeRateLimited:
		remote, _ := sdk.NewRemoteError(sdk.ErrorRateLimited, "rate_limited", "req-woo", time.Second)
		return conformance.ProbeResult{}, remote
...
func (candidateTransport) Do(_ context.Context, request Request) (Response, error) {
	...
	if request.Method == "GET" && request.Host == "shop.example.com" && request.Path == "/wp-json/wc/v3/products" {
		return Response{StatusCode: 200, Body: []byte(`[]`), TotalPages: 1}, nil
	}
...
func (candidateSecrets) UseSecret(_ context.Context, _ sdk.SecretReference, cb func([]byte) error) error {
	...
	value := []byte(`{"consumer_key":"ck_12345678901234567890123456789012","consumer_secret":"cs_12345678901234567890123456789012","webhook_secret":"whsec_123456789012345678901234567890123456"}`)
	defer clear(value)
	return cb(value)
}

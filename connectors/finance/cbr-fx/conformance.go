...
func NewConformanceCandidate(exe string) (*ConformanceCandidate, error) {
	// Task-064 sandbox probes exercise the generic secret boundary even for public/no-auth providers.
	// The synthetic secret exists only inside the harness and is not part of the CBR manifest.
	sandboxManifest := Manifest()
	sandboxManifest.Auth = []sdk.AuthRequirement{{Kind: sdk.AuthAPIKey, SecretClass: "connector_token", Required: true}}
...
func (c *ConformanceCandidate) Account(t conformance.Tenant) sdk.Account {
	...
	return sdk.Account{ID: "cbr-fx-conformance", OrganizationID: t.OrganizationID, WorkspaceID: t.WorkspaceID, ConnectorID: "cbr-fx", Family: sdk.FamilyFX, Status: sdk.AccountActive, Version: 1, Health: sdk.Health{Status: sdk.HealthUnknown}, CreatedAt: at, UpdatedAt: at}
}
...
func (c *ConformanceCandidate) Probe(_ context.Context, r conformance.ProbeRequest) (conformance.ProbeResult, error) {
	switch r.Kind {
	case conformance.ProbeAuthValid, conformance.ProbeAuthInvalid:
...
func candidateRemote(cat sdk.ErrorCategory, code string, retry time.Duration) error {
	e, _ := sdk.NewRemoteError(cat, code, "cbr-fx-conformance", retry)
	return e
}

type candidateTransport struct{}

func (candidateTransport) Daily(context.Context, time.Time) ([]byte, error) {
	return append([]byte(nil), candidateDailyXML...), nil
}

type candidateRuntime struct{}

func (candidateRuntime) Secrets() sdk.SecretAccessor { return nil }

...
Status: Repository implementation complete; bounded pickup, rate and tracking reads enabled
...
- bounded read-only `logistics.track.read` через отдельный официальный SOAP
  API отслеживания с единичным штрихкодом и отдельными tracking credentials;
...
- runtime support `separate_surface/logistics`: кабинет, health-check,
  bounded чтение ОПС, read-only тарифный предпросмотр и единичное tracking
  чтение доступны; отправления, документы и возвраты остаются fail-closed.
...
- `logistics.rates.read` использует официальный тарифный API, передаёт
  ...
  с НДС в копейки;
- `logistics.track.read` использует SOAP 1.2 `getOperationHistory`, принимает
  только внутренний 14-значный или международный S10 barcode, ограничивает
  историю ответа и возвращает только нормализованный последний статус;
...
Для включения создания/отмены отправлений, печатных форм и возвратов

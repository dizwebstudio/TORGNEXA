...
func listReports(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]any{"items": []reportDefinition{
		...
		{ID: "seller_profit_and_loss", Title: "P&L продавца", Description: "Управленческий отчёт: чистые продажи, FIFO-себестоимость, комиссии и вклад.", Source: "postgresql.snapshot", Availability: "ready", FreshnessSLASeconds: 3600, Dimensions: []string{"channel_ref", "currency", "quality_status"}, Metrics: []string{"net_sales_minor_units", "cogs_minor_units", "contribution_profit_minor_units", "margin_basis_points"}},
		{ID: "seller_cash_flow", Title: "Денежный поток продавца", Description: "Выплаты, поступления, возвраты и подтверждённые денежные расходы без двойного счёта settlement.", Source: "postgresql.snapshot", Availability: "ready", FreshnessSLASeconds: 3600, Dimensions: []string{"channel_ref", "currency", "quality_status"}, Metrics: []string{"payout_minor_units", "bank_receipt_minor_units", "net_cash_minor_units"}},
		{ID: "seller_unit_economics", Title: "Юнит-экономика продавца", Description: "Комиссии, логистика, хранение, реклама, возвраты и маржа по каналам.", Source: "postgresql.snapshot", Availability: "ready", FreshnessSLASeconds: 3600, Dimensions: []string{"channel_ref", "currency", "quality_status"}, Metrics: []string{"take_rate_basis_points", "refund_rate_basis_points", "logistics_per_order_minor_units"}},
		{ID: "seller_financial_quality", Title: "Качество финансовых данных", Description: "Покрытие расчёта и причины, по которым показатель отмечен как неполный или спорный.", Source: "postgresql.snapshot", Availability: "ready", FreshnessSLASeconds: 3600, Dimensions: []string{"channel_ref", "currency", "quality_status"}, Metrics: []string{"coverage_percent"}},
	}})
}

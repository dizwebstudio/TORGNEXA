...
func (f SpendFact) Validate() error { return validateFact(f.ID, f.AccountID, f.Channel, f.CampaignID, f.RemoteFactID, f.PeriodStart, f.PeriodEnd, f.AmountMinor, f.Currency, f.Source, f.ObservedAt, f.EffectiveAt, f.Quality) }
...
func DeduplicateSpend(facts []SpendFact) ([]SpendFact, error) {
	...
	for _, fact := range facts {
		if err := fact.Validate(); err != nil { return nil, err }
		...
		if previous, ok := seen[key]; ok {
			if FingerprintSpend(previous) != FingerprintSpend(fact) { return nil, ErrConflict }
			continue
		}
		seen[key] = fact
	}
	result := make([]SpendFact, 0, len(seen))
	for _, fact := range seen { result = append(result, fact) }
	sort.Slice(result, func(i, j int) bool { return result[i].PeriodStart.Before(result[j].PeriodStart) || (result[i].PeriodStart.Equal(result[j].PeriodStart) && result[i].ID < result[j].ID) })
	return result, nil
}
...
func Aggregate(spends []SpendFact, performance []PerformanceFact) ([]Metric, error) {
	...
	quality := func(current Quality, next Quality) Quality { if current == "" { return next }; if current == next { return current }; return QualityPartial }
	for _, fact := range spends {
		if err := fact.Validate(); err != nil { return nil, err }
		k := key{fact.Channel, fact.CampaignID, fact.SKU, fact.Currency}; m := values[k]; if m == nil { m = &Metric{Channel: fact.Channel, CampaignID: fact.CampaignID, SKU: fact.SKU, Currency: fact.Currency}; values[k] = m }
		m.SpendMinor += fact.AmountMinor; m.Quality = quality(m.Quality, fact.Quality)
	}
	for _, fact := range performance {
		if err := fact.Validate(); err != nil { return nil, err }
		k := key{fact.Channel, fact.CampaignID, fact.SKU, fact.Currency}; m := values[k]; if m == nil { m = &Metric{Channel: fact.Channel, CampaignID: fact.CampaignID, SKU: fact.SKU, Currency: fact.Currency}; values[k] = m }
		m.Impressions += fact.Impressions; m.Clicks += fact.Clicks; m.Orders += fact.Orders; m.RevenueMinor += fact.RevenueMinor; m.Quality = quality(m.Quality, fact.Quality)
	}
	result := make([]Metric, 0, len(values))
	for _, m := range values {
		if m.SpendMinor > 0 { m.ROASBasisPoints = ratio(m.RevenueMinor, m.SpendMinor); m.ROMIBasisPoints = ratio(m.RevenueMinor-m.SpendMinor, m.SpendMinor); if m.Orders > 0 { m.OrderCostMinor = m.SpendMinor / m.Orders } }
		if m.RevenueMinor > 0 { m.DRRBasisPoints = ratio(m.SpendMinor, m.RevenueMinor) }
		if m.Quality == "" { m.Quality = QualityUnknown }
		result = append(result, *m)
	}
	sort.Slice(result, func(i, j int) bool { if result[i].Channel != result[j].Channel { return result[i].Channel < result[j].Channel }; if result[i].CampaignID != result[j].CampaignID { return result[i].CampaignID < result[j].CampaignID }; return result[i].SKU < result[j].SKU })
	return result, nil
}

func ratio(numerator, denominator int64) int64 {
	if denominator <= 0 { return 0 }
	n := new(big.Int).Mul(big.NewInt(numerator), big.NewInt(10000)); n.Quo(n, big.NewInt(denominator)); if !n.IsInt64() { return 0 }; return n.Int64()
}

func validRef(value string) bool { return value != "" && len(value) <= 192 && value == strings.TrimSpace(value) && !strings.ContainsAny(value, "\x00\r\n") }
func validText(value string, max int) bool { return value != "" && len(value) <= max && value == strings.TrimSpace(value) && !strings.ContainsAny(value, "\x00\r\n") }
func validCurrency(value string) bool { return len(value) == 3 && value == strings.ToUpper(value) && value >= "A" && value <= "ZZZ" }

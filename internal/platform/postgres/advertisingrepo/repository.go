...
func New(db *sql.DB) (*Repository, error) { if db == nil { return nil, ErrInvalid }; return &Repository{db: db}, nil }

func (r *Repository) validate(ctx context.Context, scope tenancy.Scope) error { if r == nil || r.db == nil || ctx == nil || !scope.Valid() { return ErrInvalid }; return nil }
...
func (r *Repository) UpsertCampaign(ctx context.Context, scope tenancy.Scope, c core.Campaign) error {
	if err := r.validate(ctx, scope); err != nil || c.Validate() != nil { return ErrInvalid }
...
func (r *Repository) AppendSpend(ctx context.Context, scope tenancy.Scope, fact core.SpendFact) error {
	if err := r.validate(ctx, scope); err != nil || fact.Validate() != nil { return ErrInvalid }
	...
	return r.tx(ctx, scope, false, func(tx *sql.Tx) error {
		...
		if err == nil { if existing != fingerprint { return ErrConflict }; return nil }
...
func (r *Repository) ListCampaigns(ctx context.Context, scope tenancy.Scope, filter Filter) ([]core.Campaign, error) {
	if err := r.validateFilter(ctx, scope, filter); err != nil { return nil, err }
	...
	err := r.tx(ctx, scope, true, func(tx *sql.Tx) error {
		...
		for rows.Next() { var c core.Campaign; if err := rows.Scan(&c.ID,&c.AccountID,&c.Channel,&c.RemoteID,&c.Name,&c.Status,&c.Currency,&c.DailyBudgetMinor,&c.TotalBudgetMinor,&c.ObservedAt,&c.EffectiveFrom,&c.EffectiveTo,&c.Version); err != nil { return err }; if c.EffectiveFrom.Unix() == 0 { c.EffectiveFrom = time.Time{} }; if c.EffectiveTo.Unix() == 0 { c.EffectiveTo = time.Time{} }; items = append(items,c) }
		return rows.Err()
	})
	return items, err
}
...
func (r *Repository) ListSpend(ctx context.Context, scope tenancy.Scope, filter Filter) ([]core.SpendFact, error) {
	if err := r.validateFilter(ctx, scope, filter); err != nil { return nil, err }; if filter.Limit == 0 { filter.Limit = 200 }
	...
	err := r.tx(ctx, scope, true, func(tx *sql.Tx) error {
		...
		if err != nil { return err }; defer rows.Close(); for rows.Next() { var f core.SpendFact; if err := rows.Scan(&f.ID,&f.AccountID,&f.Channel,&f.CampaignID,&f.AdID,&f.SKU,&f.RemoteFactID,&f.PeriodStart,&f.PeriodEnd,&f.AmountMinor,&f.Currency,&f.Source,&f.ObservedAt,&f.EffectiveAt,&f.Quality); err != nil { return err }; items = append(items,f) }; return rows.Err()
	})
	return items, err
}
...
func (r *Repository) ListPerformance(ctx context.Context, scope tenancy.Scope, filter Filter) ([]core.PerformanceFact, error) {
	...
	err := r.tx(ctx, scope, true, func(tx *sql.Tx) error {
		...
		if err != nil { return err }; defer rows.Close(); for rows.Next() { var f core.PerformanceFact; if err := rows.Scan(&f.ID,&f.AccountID,&f.Channel,&f.CampaignID,&f.AdID,&f.SKU,&f.RemoteFactID,&f.PeriodStart,&f.PeriodEnd,&f.Impressions,&f.Clicks,&f.Orders,&f.RevenueMinor,&f.Currency,&f.Source,&f.ObservedAt,&f.EffectiveAt,&f.Quality); err != nil { return err }; items = append(items,f) }; return rows.Err()
	})
	return items, err
}
...
func (r *Repository) ListMetrics(ctx context.Context, scope tenancy.Scope, filter Filter) ([]core.Metric, error) {
	spends, err := r.ListSpend(ctx, scope, filter); if err != nil { return nil, err }; performance, err := r.ListPerformance(ctx, scope, filter); if err != nil { return nil, err }; return core.Aggregate(spends, performance)
}
...
func (r *Repository) ListFindings(ctx context.Context, scope tenancy.Scope, limit int) ([]core.Finding, error) {
	if err := r.validate(ctx, scope); err != nil || limit < 1 || limit > 200 { return nil, ErrInvalid }
	items := make([]core.Finding, 0, limit); err := r.tx(ctx, scope, true, func(tx *sql.Tx) error { rows, err := tx.QueryContext(ctx, `SELECT finding_id,kind,campaign_id,remote_reference,expected_minor,actual_minor,severity,explanation,observed_at FROM advertising_reconciliation_findings WHERE organization_id=$1 AND workspace_id=$2 ORDER BY observed_at DESC,finding_id DESC LIMIT $3`,scope.OrganizationID(),scope.WorkspaceID(),limit); if err != nil{return err}; defer rows.Close(); for rows.Next(){var f core.Finding; if err:=rows.Scan(&f.ID,&f.Kind,&f.CampaignID,&f.RemoteReference,&f.ExpectedMinor,&f.ActualMinor,&f.Severity,&f.Explanation,&f.ObservedAt);err!=nil{return err};items=append(items,f)};return rows.Err() }); return items,err
}
...
func (r *Repository) StartSyncRun(ctx context.Context, scope tenancy.Scope, run SyncRun) (SyncRun, error) {
	if err := r.validate(ctx, scope); err != nil || !validSyncRun(run) { return SyncRun{}, ErrInvalid }
	if run.CreatedAt.IsZero() { run.CreatedAt = time.Now().UTC() }; if run.ID == "" { sum := sha256.Sum256([]byte(run.AccountID+"\x00"+run.Channel+"\x00"+run.From.Format(time.RFC3339)+"\x00"+run.To.Format(time.RFC3339)+"\x00"+run.Mode)); run.ID = "adsrun-"+hex.EncodeToString(sum[:])[:24] }
...
func (r *Repository) UpdateSyncRun(ctx context.Context, scope tenancy.Scope, run SyncRun) error {
	if err := r.validate(ctx, scope); err != nil || !validSyncRun(run) || run.ID == "" || len(run.NextCursor) > 4096 { return ErrInvalid }
	return r.tx(ctx, scope, false, func(tx *sql.Tx) error { _,err:=tx.ExecContext(ctx,`UPDATE advertising_sync_runs SET status=$4,next_cursor=$5,watermark_at=$6,fetched_count=$7,accepted_count=$8,rejected_count=$9,error_code=$10,evidence_digest=$11,completed_at=$12 WHERE organization_id=$1 AND workspace_id=$2 AND run_id=$3`,scope.OrganizationID(),scope.WorkspaceID(),run.ID,run.Status,run.NextCursor,nullableTime(run.WatermarkAt),run.FetchedCount,run.AcceptedCount,run.RejectedCount,run.ErrorCode,nullableString(run.EvidenceDigest),nullableTime(run.CompletedAt));return err })
}

func (r *Repository) validateFilter(ctx context.Context, scope tenancy.Scope, f Filter) error { if err:=r.validate(ctx,scope);err!=nil{return err}; if f.Limit<0||f.Limit>200||len(f.Channel)>64||len(f.CampaignID)>192||len(f.SKU)>200||( !f.From.IsZero() && f.From.Location()!=time.UTC)||(!f.To.IsZero()&&f.To.Location()!=time.UTC)||(!f.From.IsZero()&&!f.To.IsZero()&&!f.To.After(f.From)){return ErrInvalid};return nil }
func (r *Repository) tx(ctx context.Context, scope tenancy.Scope, readOnly bool, fn func(*sql.Tx) error) error { opts:=&sql.TxOptions{ReadOnly:readOnly};tx,err:=r.db.BeginTx(ctx,opts);if err!=nil{return err};defer tx.Rollback();if _,err=tx.ExecContext(ctx,applyScope,scope.OrganizationID().String(),scope.WorkspaceID().String());err!=nil{return err};if err=fn(tx);err!=nil{return err};return tx.Commit() }
func validFinding(f core.Finding) bool { return f.ID!=""&&f.Kind!=""&&f.Severity!=""&&len(f.Explanation)>=1&&len(f.Explanation)<=500&&!f.ObservedAt.IsZero()&&f.ObservedAt.Location()==time.UTC }
func validSyncRun(r SyncRun) bool { return r.AccountID!=""&&r.Channel!=""&&!r.From.IsZero()&&!r.To.IsZero()&&r.From.Location()==time.UTC&&r.To.Location()==time.UTC&&r.To.After(r.From)&&(r.Mode=="daily"||r.Mode=="incremental"||r.Mode=="backfill")&&(r.Status==""||r.Status=="queued"||r.Status=="running"||r.Status=="completed"||r.Status=="partial"||r.Status=="failed"||r.Status=="dead_letter")&&r.FetchedCount>=0&&r.AcceptedCount>=0&&r.RejectedCount>=0 }

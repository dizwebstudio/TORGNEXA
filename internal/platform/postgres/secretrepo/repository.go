...
// WithinTransaction joins a caller's tenant/pool-bound transaction, allowing
// encrypted secret lifecycle changes, account binding and audit to commit once.
...
// non-rollbackable provider refresh call. Secret references and token material
// are intentionally absent from the persisted row.
func (repository *Repository) RecordRefreshIntent(ctx context.Context, scope tenancy.Scope, intent secrets.RefreshIntent) error {
	...
	return repository.readWrite(ctx, scope, func(tx *sql.Tx) error {
		...
		if err != nil {
			return fmt.Errorf("record oauth refresh intent result: %w", err)
		}
		if rows == 1 {
			return nil
		}

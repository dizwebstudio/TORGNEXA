...
func (k Kind) Valid() bool {
	return k == KindReconciliation || k == KindUpload || k == KindPrivacy || k == KindWarehouseIncident
}
...
func (r *Repository) Claim(ctx context.Context, kind Kind, workerID string, batch int, lease time.Duration) ([]Job, error) {
	...
	if err != nil {
		...
		if (kind == KindPrivacy || kind == KindWarehouseIncident) && errors.As(err, &state) && state.SQLState() == "22023" {

...
Миграция не требуется. Используются существующие tenant-scoped записи
`return_logistics` и operation receipt; в них сохраняются только нормализованные
идентификаторы, статус и код ошибки. Provider credentials, description и
сырой ответ не записываются.

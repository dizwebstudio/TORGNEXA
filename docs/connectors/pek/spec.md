...
`logistics.shipment.create`, `logistics.shipment.cancel` и `logistics.label.read`.
...
нормализуется в нейтральный статус. `logistics.label.read` с форматом `pdf`
...
груза, возврат и пакетная печать (`type=multiple`) остаются закрыты.

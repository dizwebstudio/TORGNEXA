...
нормализуется в нейтральный статус. Расчёт, оформление, этикетки и операции
отправлений остаются закрытыми до квалификации актуальных fixtures и
идемпотентного host-side bridge.

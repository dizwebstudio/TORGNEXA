...
Configuration contains only a lower-case portal host such as `acme.bitrix24.com`. Task 134 resolves the encrypted OAuth bundle at the host boundary and supplies only raw current access-token bytes through `SecretAccessor`; the token is sent in `Authorization: Bearer` and is not appended to the URL or request JSON. Refresh/client material never enters this provider package.

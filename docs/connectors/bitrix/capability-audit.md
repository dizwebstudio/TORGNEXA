...
| `inventory.read` | admitted | SDK adapter reads active warehouses through `catalog.store.list` and integer balances through `catalog.storeproduct.list`; warehouse identity remains explicit; reconciliation imports mapped warehouse/offer pairs through the canonical inventory repository |
...
| `prices.read` | admitted | SDK adapter reads the configured regular price type through `catalog.price.list`; runtime configuration requires `price_type_id`; reconciliation imports mapped offer prices with exact currency minor units |

...
- `PUT /api/2.0/orders/{order_id}` — смена стандартного статуса заказа;
...
`prices.write` обновляет `price` и `list_price`, а `inventory.write` обновляет
целочисленный `amount`; оба маршрута используют PUT существующего товара,
идемпотентный локальный ключ и повторное чтение. `orders.status.write`
ограничен стандартными кодами CS-Cart: `pending → O`, `confirmed → Y`,
`processing → P`, `fulfilled → C`, `cancelled → I`; перед PUT выполняется
чтение заказа, после PUT — read-after-write. Создание и удаление заказов,
произвольные пользовательские статусы и уведомления по e-mail не заявляются.
...
цен на option combinations намеренно не заявляются до появления отдельного

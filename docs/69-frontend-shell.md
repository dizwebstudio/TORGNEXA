...
The account profile presents the provider-owned `email` and `username` plus the tenant-scoped `birthdate`, `job_title`, `department`, `phone_number` and avatar projection from `GET/PATCH /api/v1/me/profile`. The demo Keycloak user supplies synthetic values and a local avatar so the Community contour remains usable offline. Profile edits are versioned, audited and stored in PostgreSQL; uploaded avatars remain behind the quarantine, malware-scan and release gate, while Keycloak continues to own passwords and provider identity.
...
- Settings → tabbed account/workspace administration with dedicated `Доступ` and `Каналы и важность` tabs; current OIDC principal, session expiry and provider-owned password/security management remain under `Основные`, while roles, capabilities and workspace members live under `Доступ`.

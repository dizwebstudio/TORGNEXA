...
if command -v go >/dev/null 2>&1; then
  go test ./internal/core/marketplacelisting ./internal/core/marketplacepublication ./internal/platform/connectors ./internal/platform/builtinruntime ./internal/platform/postgres/marketplacelistingrepo ./internal/platform/postgres/marketplacepublicationrepo ./internal/app/api ./internal/app/worker ./internal/app/mcp ./connectors/marketplaces/wildberries ./connectors/marketplaces/ozon ./connectors/marketplaces/yandex-market
else
  command -v docker >/dev/null 2>&1 || { echo "marketplace listing qualification: Go or Docker is required" >&2; exit 1; }
  docker run --rm -v "$root":/app -w /app golang:1.26.7-alpine3.23 sh -c 'export GOTOOLCHAIN=local GOWORK=off TZ=UTC; go test ./internal/core/marketplacelisting ./internal/core/marketplacepublication ./internal/platform/connectors ./internal/platform/builtinruntime ./internal/platform/postgres/marketplacelistingrepo ./internal/platform/postgres/marketplacepublicationrepo ./internal/app/api ./internal/app/worker ./internal/app/mcp ./connectors/marketplaces/wildberries ./connectors/marketplaces/ozon ./connectors/marketplaces/yandex-market'
fi
...
if evidence_schema_document.get('$id') != 'https://torgnexa.local/schemas/qualification/marketplace-remote-evidence-v1.json' or evidence_schema_document.get('properties', {}).get('schema_version', {}).get('const') != 1:
...
for path, marker in {
    'connectors/marketplaces/wildberries/taxonomy.go': 'ReadMarketplaceListingTaxonomy',
    'connectors/marketplaces/ozon/taxonomy.go': 'ReadMarketplaceListingTaxonomy',
}.items():
    if marker not in (root / path).read_text():
        raise SystemExit(f'marketplace listing qualification: provider taxonomy adapter missing: {path}')
taxonomy = (root / 'internal/platform/builtinruntime/listing_taxonomy.go').read_text()
...
print('Marketplace listing synthetic qualification: PASS — provider taxonomy adapters, conditional attributes, bounded 1000-SKU preview, approval journal, UI and read-after-write contracts')
print('Live marketplace qualification: REQUIRED at release — credentialed remote batch write and read-after-write evidence are not synthesized')
PY

...
POSTGRES_PORT=5432
KAFKA_PORT=9092
# kafka-init создаёт канонические base/retry/DLQ topics перед worker.
TORGNEXA_KAFKA_TOPIC_PARTITIONS=1
TORGNEXA_KAFKA_TOPIC_REPLICATION_FACTOR=1
VALKEY_PORT=6379
...
# Community Compose запускает ClamAV и worker обрабатывает изображения по умолчанию.

...
def parse_plan(path: str):
    ...
    for x in items:
        ...
        unknown_account=set(x)-{"account_id","connector_id","run_sync"}
        ...
        ids.add(aid); out.append({"account_id":aid,"connector_id":cid,"run_sync":run_sync})
    return {"accounts":out}
...
def main():
    ...
    for spec in wanted:
        ...
        results.append({'account_id':spec['account_id'],'connector_id':spec['connector_id'],'health_status':'healthy','health_category':'healthy','consecutive_healthy_checks':2,'checked_at':snapshots[0].get('checked_at'),'sync_started':sync_started})

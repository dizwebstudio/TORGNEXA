...
CONNECTOR_KINDS = ("marketplace", "carrier", "payment", "fiscal")
ARTIFACT_KINDS = (
    "marketplace-remote",
...
def validate_document(data: Any) -> None:
    ...
    if not isinstance(connectors, dict) or set(connectors) != set(CONNECTOR_KINDS):
        fail("connectors must contain exactly marketplace, carrier, payment and fiscal")
    ...
    if not isinstance(artifacts, list) or len(artifacts) != len(ARTIFACT_KINDS):
        fail("artifacts must contain exactly five retained evidence artifacts")
    ...
    if set(artifact_by_kind) != set(ARTIFACT_KINDS):
        fail("all marketplace, carrier, payment and fiscal artifacts are required")
...
def validate_bundle(
    ...
    """Validate the aggregate and all five retained evidence artifacts."""
    ...
    if set(evidence_paths) != set(ARTIFACT_KINDS):
        fail("all five evidence paths are required")
    ...
    if live.get("status") != "PASS" or live.get("scope") != "qualification":
        fail("marketplace live smoke must be a credentialed qualification PASS")
    ...
    for kind in ("carrier", "payment", "fiscal"):
        connector_golden_path_evidence.validate(loaded[kind], kind)
        if loaded[kind].get("connector_id") != connector_identity[kind]["connector_id"]:
            fail(f"{kind} evidence connector does not match the manifest")
        if loaded[kind].get("account_ref") != connector_identity[kind]["account_ref"]:
...
def main() -> int:
    ...
    parser.add_argument("--marketplace-live-smoke", required=True, help="retained marketplace live smoke JSON")
    ...
    print("Credentialed marketplace, carrier, payment and fiscal artifacts are bound to this release.")
    return 0

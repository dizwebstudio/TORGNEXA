...
type R={body:any};type Client={listMCPAccounts():Promise<R>;createMCPAccount(x:object):Promise<R>;disableMCPAccount(x:object):Promise<R>;getMCPAccountPolicy(x:object):Promise<R>;installMCPAccountPolicy(x:object):Promise<R>;getMCPAgentKillSwitch():Promise<R>;setMCPAgentKillSwitch(x:object):Promise<R>};
...
function AgentKillSwitch({api,canWrite}:{api:Client;canWrite:boolean}){
 ...
 return <>
  {disabled?<div className="incident-banner"><Icon name="incident"/><div><strong>Все AI-агенты остановлены аварийным выключателем.</strong><p>Вызовы MCP-инструментов блокируются для всех агентов этого workspace независимо от их индивидуальных политик, пока выключатель не будет снят.</p></div>{canWrite?<button className="button primary" onClick={()=>setConfirming(true)}>Возобновить работу</button>:null}</div>
   :canWrite?<div className="settings-card-heading"><div><p className="eyebrow">Аварийный выключатель</p><span className="settings-copy">Мгновенно останавливает вызовы всех AI-агентов этого workspace, независимо от их политик.</span></div><button className="button ghost danger-text" onClick={()=>setConfirming(true)}>Остановить всех агентов</button></div>:null}
...
function AccountPolicyPanel({api,account,canWrite}:{api:Client;account:Account;canWrite:boolean}){
 ...
 if(query.isError)return <ErrorBlock>Не удалось загрузить политику.</ErrorBlock>;

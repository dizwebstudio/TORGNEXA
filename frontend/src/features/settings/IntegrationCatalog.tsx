...
export function IntegrationCatalog(){
 ...
 return <section className="settings-section">
  ...
  <Drawer open={!!selected} title={selected?.name??"Интеграция"} subtitle={selected?`${familyLabels[selected.family]??selected.family} · ${authLabels[selected.authKinds[0]]??selected.authKinds[0]}`:undefined} onClose={()=>setSelected(null)}><>{selected?<div className="connector-drawer">{selected.runtime.stage==="planned"?<section className="drawer-section runtime-availability-note"><StatusBadge value="planned"/><h3>Подключение пока недоступно</h3><p className="drawer-help">Для адаптера проверены манифест и SDK-контракт, но в текущем production runtime нет исполняемого domain bridge. Создать кабинет или включить заявленные возможности нельзя.</p></section>:selected.runtime.stage==="separate_surface"&&selected.runtime.surface!=="social"?<SeparateSurfaceNotice entry={selected} close={()=>setSelected(null)}/>:<>{selected.runtime.surface==="social"?<section className="drawer-section runtime-availability-note"><StatusBadge value="active"/><h3>Текстовые публикации работают</h3><p className="drawer-help">Сохраните bot token, заполните chat_id из шаблона настройки, проверьте права бота, включите social.post.text и активируйте кабинет. Точный лимит текста зависит от провайдера.</p><button className="button secondary" onClick={()=>window.location.assign("/social")}>Открыть публикации</button></section>:null}<section className="drawer-section"><h3>Добавить кабинет</h3><p className="drawer-help">Создайте логическое подключение. Секреты будут сохранены отдельно в SecretProvider.</p><div className="drawer-inline"><input value={accountId} onChange={event=>setAccountId(event.target.value)} placeholder="main-account"/><button className="button primary" disabled={create.isPending||!accountIDPattern.test(accountId.trim())} onClick={()=>create.mutate(selected.id)}>Добавить</button></div></section>{(accounts.data??[]).filter(account=>account.connector_id===selected.id).map(account=><AccountCard key={account.id} account={account} entry={selected} credential={credentials[account.id]??""} setCredential={value=>setCredentials(current=>({...current,[account.id]:value}))} enabled={capDrafts[account.id]??account.capabilities.filter(value=>value.enabled).map(value=>value.capability)} setEnabled={value=>setCapDrafts(current=>({...current,[account.id]:value}))} enroll={()=>enroll.mutate(account)} saveCapabilities={enabled=>capabilities.mutate({account,enabled})} check={()=>check.mutate(account)} enable={()=>enable.mutate(account)} disable={()=>disable.mutate(account)} sync={()=>sync.mutate(account)} oauth={()=>startOAuth.mutate(account)} busy={enroll.isPending||capabilities.isPending||check.isPending||enable.isPending||disable.isPending||sync.isPending||startOAuth.isPending}/>)}</>}</div>:null}</></Drawer>
 </section>;
}

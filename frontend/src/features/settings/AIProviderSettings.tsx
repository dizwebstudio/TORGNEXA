...
type ProviderID="openai-compatible"|"gigachat"|"yandexgpt"|"kimi"|"qwen"|"deepseek"|"claude"|"gemini"|"grok"|"ollama"|"lm-studio"|"open-webui";
 ...
 return <section id="ai-provider-settings" className="panel settings-card">
  ...
  {canWrite?<section className="drawer-section">
    ...
    <div className="settings-grid ai-provider-form">
     ...
     <label className="field"><span>Модель</span><input value={model} maxLength={120} placeholder={provider==="claude"?"claude-sonnet-4-20250514":provider==="gemini"?"gemini-3.7-flash":provider==="grok"?"grok-4.6":"gpt-4o-mini"} onChange={e=>setModel(e.target.value)}/></label>
     {hostOverrideProviders.has(provider)?<label className="field"><span>Base URL (необязательно)</span><input value={baseUrl} maxLength={2039} placeholder={provider==="ollama"||provider==="lm-studio"||provider==="open-webui"?localBaseURLPlaceholders[provider]:provider==="claude"?"https://api.anthropic.com":provider==="grok"?"https://api.x.ai":"https://api.moonshot.ai"} onChange={e=>setBaseUrl(e.target.value)}/></label>:null}
     {provider==="yandexgpt"?<label className="field"><span>Идентификатор папки</span><input value={folderId} maxLength={120} placeholder="b1gxxxxxxxxxxxxxxxx" onChange={e=>setFolderId(e.target.value)}/></label>:null}
    ...
    <div className="account-actions ai-provider-form-actions"><button className="button primary" disabled={create.isPending||!valid} onClick={()=>create.mutate()}>{create.isPending?"Добавляем…":"Добавить аккаунт"}</button></div>
  </section>:null}
 </section>
}

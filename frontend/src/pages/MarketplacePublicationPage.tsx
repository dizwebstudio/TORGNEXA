...
type Drift={type:string;snapshot_id:string;remote_id?:string;remote_operation_id?:string;expected_digest?:string;observed_digest?:string;observed_state?:string;detected_at:string};
type Client={
...
export function MarketplacePublicationPage(){
  ...
  const [filter,setFilter]=useState("all"),[selected,setSelected]=useState<string>(),[showStart,setShowStart]=useState(false),[preflightReady,setPreflightReady]=useState(false);
  ...
  const preflight=useMutation({mutationFn:()=>{const body=JSON.parse(snapshotText) as unknown;return api.preflightMarketplacePublication({body:{snapshot:body,quality_receipt_id:qualityID.trim()}})},onSuccess:result=>{setPreflightReady(true);toast.push({kind:"success",title:"Предпроверка пройдена",body:`Решение: ${String((result.body as {decision?:string}).decision??"ready")}`})},onError:()=>{setPreflightReady(false);toast.push({kind:"error",title:"Предпроверка не пройдена",body:"Проверьте JSON snapshot, quality receipt, права и доступность кабинета."})}});
  const enqueue=useMutation({mutationFn:()=>{const body=JSON.parse(snapshotText) as unknown;return api.enqueueMarketplacePublication({idempotencyKey:crypto.randomUUID(),approvalRequestID:approvalID.trim()||undefined,body:{operation,snapshot:body,quality_receipt_id:qualityID.trim(),dry_run:dryRun}})},onSuccess:async()=>{toast.push({kind:"success",title:dryRun?"Dry-run поставлен в очередь":"Публикация поставлена в очередь"});setShowStart(false);setPreflightReady(false);setSnapshotText("");await refresh()},onError:()=>toast.push({kind:"error",title:"Операция не создана",body:"Проверьте snapshot, quality receipt, права и approval для live-записи."})});
  ...
  return <Page eyebrow="Каналы и каталог" title="Публикация товаров" description="Одна очередь для WB, Ozon и Yandex Market: snapshot проходит проверку до внешней записи, а неопределённый результат остаётся на контроле оператора." actions={canWrite?<button type="button" className="button primary" onClick={()=>setShowStart(value=>!value)}>{showStart?"Закрыть форму":"Новая операция"}</button>:null}>
    ...
    {showStart&&canWrite?<section className="panel inline-create"><div><p className="eyebrow">Проверенный snapshot</p><h2>Запустить публикацию</h2><p>Вставьте JSON snapshot из PIM/catalog pipeline. Токены и внешние URL сюда не добавляются: медиа передаются только как ReleasedObjectRef.</p></div><form onSubmit={event=>{event.preventDefault();if(preflightReady){if(!preflight.isPending&&!enqueue.isPending)enqueue.mutate()}else if(!preflight.isPending)preflight.mutate()}}><div className="form-grid"><label className="field"><span>Операция</span><select value={operation} onChange={event=>setOperation(event.target.value)}>{Object.entries(kinds).filter(([key])=>key!=="status_read").map(([key,label])=><option value={key} key={key}>{label}</option>)}</select></label><label className="field"><span>ID quality receipt</span><input required value={qualityID} onChange={event=>{setPreflightReady(false);setQualityID(event.target.value)}} placeholder="receipt_…"/></label><label className="field"><span>ID approval (для live)</span><input value={approvalID} onChange={event=>setApprovalID(event.target.value)} placeholder="approval_…" disabled={dryRun}/></label></div><label className="field"><span>Snapshot JSON</span><textarea required rows={8} value={snapshotText} onChange={event=>{setPreflightReady(false);setSnapshotText(event.target.value)}} placeholder={'{"id":"…","target":{…},"version":1,…}'}/></label><label className="checkbox-field"><input type="checkbox" checked={dryRun} onChange={event=>setDryRun(event.target.checked)}/><span>Только dry-run — не вызывать marketplace API</span></label><div className="page-actions"><button type="button" className="button ghost" disabled={preflight.isPending||!snapshotText.trim()||!qualityID.trim()} onClick={()=>preflight.mutate()}>{preflight.isPending?"Проверяем…":"Проверить snapshot"}</button>{dryRun||preflightReady?<button type="submit" className="button primary" disabled={enqueue.isPending||!snapshotText.trim()||!qualityID.trim()||(!dryRun&&!approvalID.trim())}>{enqueue.isPending?"Ставим в очередь…":dryRun?"Поставить dry-run":"Запустить публикацию"}</button>:<span className="form-hint">Сначала пройдите предпроверку snapshot для live-записи.</span>}</div></form></section>:null}

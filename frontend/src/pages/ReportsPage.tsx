...
function cell(value:string,key:string,row:string[]){if(key.endsWith("_minor_units")||key==="gross_minor_units")return value===""?"—":new Intl.NumberFormat("ru-RU",{style:"currency",currency:row[1]||"RUB",maximumFractionDigits:2}).format(Number(value)/100);if(key==="margin_basis_points")return `${(Number(value)/100).toLocaleString("ru-RU",{maximumFractionDigits:2})}%`;if(key==="coverage_percent")return `${value}%`;if(key.endsWith("_at"))return new Date(value).toLocaleString("ru-RU");return value}

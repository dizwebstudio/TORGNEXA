...
export function ReplenishmentPage() {
  ...
  const [sku, setSku] = useState(""); const [offerID, setOfferID] = useState(""); const [warehouseID, setWarehouseID] = useState(""); const [supplierOfferID, setSupplierOfferID] = useState(""); const [demand, setDemand] = useState("0"); const [returns, setReturns] = useState("0"); const [opening, setOpening] = useState("0"); const [inbound, setInbound] = useState("0"); const [horizon, setHorizon] = useState("30"); const [leadTime, setLeadTime] = useState("7");
  const query = useQuery({queryKey: ["replenishment"], queryFn: async () => decodeRuns((await api.listReplenishmentRuns({limit: 50})).body), staleTime: 15_000});
  const create = useMutation({mutationFn: () => { const unit = "PCS"; const body = {run_id: uuidV7(), horizon_days: Number(horizon), offer_id: offerID.trim(), sku: sku.trim(), warehouse_id: warehouseID.trim(), supplier_offer_id: supplierOfferID.trim(), unit, demand: {coefficient: Number(demand), scale: 0, unit}, returns: {coefficient: Number(returns), scale: 0, unit}, opening_available: {coefficient: Number(opening), scale: 0, unit}, confirmed_inbound: {coefficient: Number(inbound), scale: 0, unit}, lead_time_days: Number(leadTime)}; return api.createReplenishmentRun({idempotencyKey: uuidV7(), body}); }, onSuccess: async () => { toast.push({kind: "success", title: "Прогноз рассчитан", body: "Рекомендация сохранена как derived fact. Заказ поставщику сам не отправляется."}); await cache.invalidateQueries({queryKey: ["replenishment"]}); }, onError: () => toast.push({kind: "error", title: "Прогноз не рассчитан", body: "Проверьте SKU, склад, оффер и неотрицательные количества."})});

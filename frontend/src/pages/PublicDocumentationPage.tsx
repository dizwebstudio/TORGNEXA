    ...
    <div className="docs-layout">
      ...
      <main className="docs-content">
        ...
        <DocSection id="inventory-incidents" title="Остатки и инциденты" intro="Остатки отражают доступное количество, резервы, состояние складов и исключения fulfillment. Для кодов маркировки предусмотрен отдельный раздел «Маркировка» со сканированием и контролем партий.">
          ...
          <div><h3>Карточки marketplace</h3><ul><li><code>/catalog</code> — рабочая карточка Product/PIM: текст, SKU, цены, категории и изображения.</li><li><code>/publication-quality</code> проверяет готовность, <code>/marketplace-listings</code> даёт taxonomy и batch preview до 1 000 SKU, а <code>/marketplace-publications</code> запускает snapshot preflight и показывает статус публикации/reconciliation.</li><li><code>/catalog/bulk</code> объединяет выбор SKU и channel projections, сравнение, typed-изменения контента/локализаций, атрибутов, вариантов, media, цен и остатков, before/after diff, quality blockers, approval, partial apply и reconciliation.</li><li>Неподтверждённые каналы остаются read-only/qualification-required: provider-specific атрибуты и массовая запись не маскируются универсальной кнопкой, MCP доступен только для dry-run preview.</li></ul></div>

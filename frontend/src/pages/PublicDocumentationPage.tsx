...
function IntegrationConnectionGuide() {
  return <>
    ...
    <div className="docs-table-wrap"><table className="docs-route-table"><thead><tr><th>Карточка</th><th>Учётные данные</th><th>Параметры среды / результат проверки</th></tr></thead><tbody>
      ...
      <tr><td><strong>СДЭК / ПЭК / Почта России</strong></td><td>Учётные данные провайдера</td><td>После <code>healthy</code> доступны отдельные read-only операции: тарифы, отслеживание и ПВЗ. У СДЭК дополнительно доступна отмена отправления; создание отправления и этикетки остаются за пределами текущего маршрута.</td></tr>
      <tr><td><strong>Деловые Линии / 5Post / Ozon Доставка</strong></td><td>Учётные данные провайдера</td><td>Сейчас доступно создание кабинета и официальная проверка доступности. Заявленные в SDK отправления, тарифы, трекинг и этикетки не включаются без runtime-квалификации.</td></tr>
    ...
    <Callout title="Логистика в отдельной панели">Для СДЭК, ПЭК и Почты России после успешной проверки и активации карточка показывает доступные операции тарифа, трекинга и ПВЗ. Состав кнопок строится из текущего runtime-каталога, поэтому SDK-возможность сама по себе не означает, что операция доступна оператору.</Callout>
  </>;
}

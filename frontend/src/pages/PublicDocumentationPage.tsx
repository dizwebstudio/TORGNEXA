...
const routes = [
  ...
  ["Массовый каталог", "/catalog/bulk", "Единый multi-channel scope, preview/diff, quality, approval и построчная история"],
  ...
  ["Операции marketplace", "/marketplace-operations", "Фактические возможности кабинетов, состояние синхронизации и qualification evidence"],
  ["Заказы", "/orders", "Поиск, статусы и подробности заказов"],
  ["Возвраты", "/returns", "Возвраты, отмены, инспекции и связанные refunds"],
  ["Клиентский сервис", "/customer-service", "Единый inbox, отзывы, вопросы, претензии, SLA и ответы с контролем доставки"],
  ["Остатки", "/inventory", "Позиции, склады, инциденты, fulfillment и импорт"],
  ["Мобильный склад", "/warehouse/mobile", "FBS pick, сканирование, упаковка, печать, offline и FBO remote visibility"],
  ...
  ["Реклама", "/advertising", "Кампании, акции, ставки, бюджеты и качество рекламной аналитики"],
    ...
    <div className="docs-layout">
      ...
      <main className="docs-content">
        ...
        <DocSection id="inventory-incidents" title="Остатки и инциденты" intro="Остатки отражают доступное количество, резервы, состояние складов и исключения fulfillment. Для кодов маркировки предусмотрен отдельный раздел «Маркировка» со сканированием и контролем партий.">
          ...
          <div><h3>Прогноз и пополнение</h3><ul><li>Раздел <code>/replenishment</code> принимает нормализованные спрос, возвраты, доступный запас, подтверждённый inbound и supplier offer.</li><li>Результат показывает digest, качество данных, p50/p90, shortfall, срок поставки и объяснимые reason codes.</li><li>Текущий режим — <code>recommendation_only</code>: он не меняет WMS и не отправляет заказ поставщику. Создание draft PO и approved submit остаются отдельным procurement шагом.</li></ul></div>
          <div><h3>Цены и repricing</h3><ul><li>Раздел <code>/pricing</code> запускает детерминированный dry-run для массовых ценовых решений.</li><li>Каждая строка получает digest, изменение в минимальных единицах валюты и объяснение блокировки по floor price или максимальному шагу.</li><li>Preview не вызывает marketplace и не меняет цену. Apply, акции, Buy Box и реклама включаются только после отдельной квалификации и approval.</li></ul></div>
          <div><h3>Карточки marketplace</h3><ul><li><code>/catalog</code> — рабочая карточка Product/PIM: текст, SKU, цены, категории и изображения.</li><li><code>/publication-quality</code> проверяет готовность, <code>/marketplace-listings</code> даёт taxonomy и batch preview до 1 000 SKU, а <code>/marketplace-publications</code> запускает snapshot preflight и показывает статус публикации/reconciliation.</li><li>Approved remote plan передаёт выбранные eligible-строки в общий durable publication runtime: каждая строка получает immutable snapshot, quality receipt, свою idempotency key и operation ID.</li><li><code>/catalog/bulk</code> объединяет выбор SKU и channel projections, сравнение, typed-изменения контента/локализаций, атрибутов, вариантов, media, цен и остатков, before/after diff, quality blockers, approval, partial apply и reconciliation.</li><li>WB, Ozon и Yandex Market пока остаются <code>qualification_required</code> до официальной taxonomy и credentialed read-after-write проверки; MCP доступен только для dry-run preview.</li></ul></div>
        ...
        <DocSection id="social" title="Публикации" intro="Social Core хранит контент, канал, расписание и историю статусов независимо от конкретной социальной сети.">
          <p>В текущей версии рабочей среды подключены Telegram, MAX и VK для текстовых сообщений. Лимит Telegram — 4096 символов, MAX — 4000, VK — 16384. В «Интеграциях» создайте кабинет: для Telegram/MAX сохраните токен бота и заполните <code>chat_id</code>, для VK пройдите OAuth и укажите <code>group_id</code>. Выполните проверку, включите <code>social.post.text</code> и активируйте кабинет. Затем создайте активный канал и публикацию в разделе «Публикации».</p>
        ...
        <DocSection id="settings" title="Настройки" intro="Настройки разделены на семь вкладок; состав доступных действий зависит от роли администратора.">
          ...
          <p>Во вкладке «Основные» текущий пользователь видит рабочее пространство, срок сессии и профиль, а во вкладке «Доступ» — назначенные роли, разрешения и участников workspace. Изменение личных полей использует optimistic version и ключ идемпотентности; конфликт версии означает, что сначала нужно перечитать профиль.</p>
          ...
          <div className="docs-table-wrap"><table className="docs-route-table"><thead><tr><th>Вкладка</th><th>Что находится внутри</th></tr></thead><tbody>
            <tr><td><strong>Основные</strong></td><td>Профиль, способы входа, рабочее пространство, подписка, сессии, интеграции и провайдеры ИИ.</td></tr>
            <tr><td><strong>Доступ</strong></td><td>Роли, разрешения и участники рабочего пространства с администрированием профилей.</td></tr>
        ...
        <DocSection id="operations" title="Эксплуатация Community-контура" intro="Перед обновлением проверьте состояние сервисов, резервные копии и возможность восстановления.">
          ...
          <h3>Акции и реклама</h3>
          <p>В разделе «Реклама» доступны кампании, акции, ставки и бюджеты, массовый preview до 1 000 SKU, расходы и сверка. Любая запись начинается с расчёта effective price, floor и маржи, затем требует согласованный запрос и ключ идемпотентности.</p>
          <p>Состояние <code>qualification_required</code> означает, что локальная проверка и approval завершены, но официальный remote write конкретного кабинета ещё не подтверждён credentials и read-after-write. Это не сообщение об успешном изменении в WB или Ozon.</p>
          <p>При сомнительном результате включите kill switch, не повторяйте операцию вслепую и откройте вкладку «Сверка». Финансовый ledger исправляется только корректирующей записью, а не удалением evidence.</p>
        </DocSection>

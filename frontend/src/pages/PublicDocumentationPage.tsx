...
function IntegrationConnectionGuide() {
  return <>
    ...
    <Callout title="Логистика в отдельной панели">Для СДЭК, ПЭК и Почты России после успешной проверки и активации карточка показывает доступные операции тарифа, трекинга и ПВЗ. Для СДЭК при включённой capability также доступна approval-bound форма создания отправления. Состав кнопок строится из текущего runtime-каталога, поэтому SDK-возможность сама по себе не означает, что операция доступна оператору.</Callout>
  </>;
}

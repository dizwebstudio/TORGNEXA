...
export function CommandPalette({open,onClose}:{open:boolean;onClose:()=>void}){
 ...
 if(!open)return null;return <div className="command-layer" role="presentation"><button className="command-backdrop" tabIndex={-1} onClick={onClose} aria-label="Закрыть"/><div ref={paletteRef} className="command-palette" role="dialog" aria-modal="true" aria-label="Поиск и быстрый переход" tabIndex={-1}><div className="command-search"><Icon name="search"/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Раздел, заказ, товар или интеграция…" aria-label="Глобальный поиск на сервере"/><kbd>Esc</kbd></div><div className="command-results">{nav.length?<div className="command-group-label">Разделы</div>:null}{nav.map(item=><button key={item.id} onClick={()=>{navigate(item.path);onClose()}}><span><Icon name={item.icon}/><strong>{item.label}</strong></span><small>{item.shortcut??""}</small></button>)}{enabled?<div className="command-group-label">Данные · поиск на сервере {searching?"…":""}</div>:null}{entities.map(item=><button key={`${item.kind}-${item.id}`} onClick={()=>{navigate(item.path);onClose()}}><span><Icon name={item.kind==="product"?"catalog":item.kind==="order"?"orders":"connectors"}/><span className="command-entity"><strong>{item.label}</strong><small>{item.meta}</small></span></span><Icon name="chevron"/></button>)}{nav.length===0&&entities.length===0&&!searching?<p>{query.trim().length<2?"Введите минимум 2 символа для поиска по серверным данным.":"Ничего не найдено"}</p>:null}</div></div></div>
}

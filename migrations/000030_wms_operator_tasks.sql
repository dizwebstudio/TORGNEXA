...
CREATE TABLE wms_execution_tasks (
  ...
  CONSTRAINT wms_execution_tasks_ref_chk CHECK (task_id ~ '^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' AND idempotency_key ~ '^[A-Za-z0-9][A-Za-z0-9._:/-]{0,191}$' AND sku ~ '^[A-Za-z0-9][A-Za-z0-9._:/-]{0,127}$' AND warehouse_id ~ '^[A-Za-z0-9][A-Za-z0-9._:/-]{0,191}$'),
...
CREATE TABLE wms_execution_task_events (
  ...
  idempotency_key text NOT NULL,
  kind text NOT NULL,
  barcode_digest char(64) NOT NULL DEFAULT '',

...
-- Task 169.13: durable lease materialization for queued assistant runs. The
-- worker receives only tenant scope and run id; prompts and provider payloads
-- never cross this queue boundary.
...
ALTER TABLE worker_runtime_jobs ADD CONSTRAINT worker_runtime_jobs_kind_chk
  CHECK (kind IN ('reconciliation','upload','privacy','warehouse_incident','social_publication','operator_assistant'));

CREATE OR REPLACE FUNCTION list_worker_active_scopes(p_limit integer)
...
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public SET row_security=off AS 'BEGIN
  ...
  RETURN QUERY
    SELECT x.organization_id,x.workspace_id FROM (
      ...
      UNION ALL
      SELECT a.organization_id,a.workspace_id,MIN(a.updated_at) AS due_at
      FROM public.assistant_runs a
      WHERE a.state IN (''queued'',''retrieving_context'',''awaiting_model'',''streaming'',''awaiting_approval'',''action_queued'')
...
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public SET row_security=off AS 'BEGIN
  IF p_kind NOT IN (''reconciliation'',''upload'',''privacy'',''warehouse_incident'',''social_publication'',''operator_assistant'') OR length(p_worker) NOT BETWEEN 1 AND 128 OR p_token !~ ''^[A-Za-z0-9][A-Za-z0-9._:/-]{0,127}$'' OR p_batch NOT BETWEEN 1 AND 1000 OR p_lease_seconds NOT BETWEEN 10 AND 600 THEN
...
RETURNS boolean LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public SET row_security=off AS 'DECLARE changed integer; BEGIN
  IF p_kind NOT IN (''reconciliation'',''upload'',''privacy'',''warehouse_incident'',''social_publication'',''operator_assistant'') OR p_delay_seconds NOT BETWEEN 0 AND 86400 OR (p_error_code IS NOT NULL AND p_error_code !~ ''^[a-z][a-z0-9._-]{0,63}$'') THEN RAISE EXCEPTION USING ERRCODE=''22023'', MESSAGE=''invalid worker release''; END IF;
...
COMMENT ON FUNCTION claim_worker_runtime_jobs(text,text,text,integer,integer) IS 'Task 169 assistant runs share the durable lease boundary; only scope and run id are exposed.';

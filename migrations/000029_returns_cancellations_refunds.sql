...
CREATE FUNCTION returns_refund_allocation_guard() RETURNS trigger LANGUAGE plpgsql AS 'DECLARE refund_payment text; refund_amount bigint; refund_currency text; refund_status text; BEGIN SELECT payment_id,amount_minor_units,currency,status INTO refund_payment,refund_amount,refund_currency,refund_status FROM payment_refunds WHERE organization_id=NEW.organization_id AND workspace_id=NEW.workspace_id AND id=NEW.refund_id; IF refund_payment IS NULL OR refund_payment <> NEW.payment_id OR refund_currency <> NEW.currency OR refund_status NOT IN (''accepted'',''succeeded'',''unknown'',''manual_attention'') OR NEW.amount_minor_units > refund_amount THEN RAISE EXCEPTION USING ERRCODE = ''23514'', MESSAGE = ''refund allocation does not match refundable refund''; END IF; RETURN NEW; END';
CREATE TRIGGER refund_allocations_guard BEFORE INSERT OR UPDATE ON refund_allocations FOR EACH ROW EXECUTE FUNCTION returns_refund_allocation_guard();
...
CREATE TRIGGER return_items_order_guard BEFORE INSERT OR UPDATE ON return_items FOR EACH ROW EXECUTE FUNCTION returns_item_order_guard();

CREATE FUNCTION returns_inspection_guard() RETURNS trigger LANGUAGE plpgsql AS 'DECLARE item_return text; requested_coefficient bigint; requested_scale smallint; item_unit text; BEGIN SELECT return_id,requested_coefficient,requested_scale,unit INTO item_return,requested_coefficient,requested_scale,item_unit FROM return_items WHERE organization_id=NEW.organization_id AND workspace_id=NEW.workspace_id AND id=NEW.return_item_id; IF item_return IS NULL OR item_return <> NEW.return_id OR item_unit <> NEW.unit OR NEW.quantity_coefficient::numeric * power(10::numeric,requested_scale) > requested_coefficient::numeric * power(10::numeric,NEW.quantity_scale) THEN RAISE EXCEPTION USING ERRCODE = ''23514'', MESSAGE = ''inspection does not match return item''; END IF; RETURN NEW; END';
CREATE TRIGGER return_inspections_guard BEFORE INSERT OR UPDATE ON return_inspections FOR EACH ROW EXECUTE FUNCTION returns_inspection_guard();

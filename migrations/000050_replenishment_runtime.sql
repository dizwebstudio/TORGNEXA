...
CREATE TABLE replenishment_runtime_recommendations (
  ...
  CONSTRAINT replenishment_runtime_recommendations_ref_chk CHECK (
  ...
  CONSTRAINT replenishment_runtime_recommendations_range_chk CHECK (
  ...
  CONSTRAINT replenishment_runtime_recommendations_state_chk CHECK (eligible_mode IN ('recommendation_only','draft_po','auto_submit') AND status IN ('proposed','accepted','rejected','deferred','on_hold'))
);
CREATE INDEX replenishment_runtime_recommendations_lookup_idx
  ON replenishment_runtime_recommendations (organization_id, workspace_id, status, created_at DESC, recommendation_id DESC);
CREATE INDEX replenishment_runtime_recommendations_grain_idx
  ON replenishment_runtime_recommendations (organization_id, workspace_id, warehouse_id, sku, created_at DESC, recommendation_id DESC);

ALTER TABLE replenishment_runtime_recommendations ENABLE ROW LEVEL SECURITY;
ALTER TABLE replenishment_runtime_recommendations FORCE ROW LEVEL SECURITY;
CREATE POLICY replenishment_runtime_recommendations_tenant_all ON replenishment_runtime_recommendations FOR ALL
...
REVOKE UPDATE, DELETE, TRUNCATE ON replenishment_runtime_recommendations FROM PUBLIC;
COMMENT ON TABLE replenishment_runtime_recommendations IS 'Tenant-scoped immutable derived replenishment decisions; does not submit purchase orders.';
